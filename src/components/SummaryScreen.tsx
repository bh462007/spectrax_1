...
{/* Weekly Activity Bar Chart - Added for GSSoC Issue #49 */}
      ...
      {stats.tags && stats.tags.length > 0 && (
        <div className="animate-in glass" style={{ width: '100%', maxWidth: '600px', padding: '20px', marginBottom: '30px', background: 'rgba(157, 78, 221, 0.05)', borderStyle: 'dashed', borderColor: 'var(--neon-purple)' }}>
          <div style={{ color: 'var(--neon-purple)', fontSize: '0.65rem', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '15px', fontWeight: 800 }}>AI VISUAL HIGHLIGHTS</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
            {stats.tags.map(tag => (
              <span key={tag} style={{ background: 'rgba(157, 78, 221, 0.15)', color: '#fff', padding: '6px 14px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, border: '1px solid var(--neon-purple)', boxShadow: '0 0 10px rgba(157, 78, 221, 0.3)' }}>
                {tag.toUpperCase()}
              </span>
            ))}
          </div>
