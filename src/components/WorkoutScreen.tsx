      ...
      try {
        ...
        poseService.onResults(async (results) => {
          ...
          if (skeletalResult && skeletalResult.confidence > 0.85) {
            ...
            const detectedKey = label.includes('squat') ? 'squat'
              : label.includes('pushup') ? 'pushup'
              : label.includes('plank') ? 'plank'
              : label.includes('jumping jack') ? 'jumpingJack'
              : label.includes('bicep curl') ? 'bicepCurl' : '';

            if (detectedKey && detectedKey !== exercise.key && mutableState.current.reps < 2) {
    ...
    <div className="screen-container" style={{ background: 'var(--bg-primary)' }}>
      ...
      {mismatchError && (
        <div style={{
          ...
          background: 'rgba(255, 34, 85, 0.95)', color: '#fff', padding: '15px 40px',
          borderRadius: '40px', boxShadow: '0 0 50px rgba(255, 34, 85, 0.6)',
          zIndex: 100, border: '2px solid #fff', textAlign: 'center',
          animation: 'shake-alert 0.4s ease-in-out'
        }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 800, letterSpacing: '3px', opacity: 0.8, marginBottom: '5px' }}>EXERCISE MISMATCH</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 900 }}>YOU ARE DOING {mismatchError}!</div>
          <div style={{ fontSize: '0.6rem', marginTop: '5px' }}>SWITCHING DISABLED MID-SET</div>
        </div>
      )}
